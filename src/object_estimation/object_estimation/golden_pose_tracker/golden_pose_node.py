...
class GoldenPoseTracker(Node):
    def __init__(self):
        ...
        # YOLO Model
        yolo_path = "/home/irol/project_hj_final/src/object_estimation/resource/best.pt"
        ...
        # Publishers
        self.marker_pub = self.create_publisher(MarkerArray, "/real_box", 10)
        ...
        self.color_sub = message_filters.Subscriber(
            self, Image, "/camera/camera/color/image_rect_raw"
        )
        ...
        self.lock_new_boxes = (
            True  # True: 처음 인식된 상자만 유지, False: 계속 새로운 상자 추가 허용
        )
        self.first_detection_time = None
        self.grace_period = 3.0  # 처음 인식 후 3초 동안만 추가 등록 허용

        self.get_logger().info(
            "Golden Pose Tracker Initialized. Waiting for camera info and images..."
        )
    ...
    def camera_info_callback(self, msg):
        if self.K is None:
            ...
            self.get_logger().info(
                f"Camera Info received. Frame: {self.camera_frame}, Resolution: {self.img_width}x{self.img_height}"
            )
        ...
        for i, det_box in enumerate(current_frame_boxes):
            ...
            if best_iou > 0.3:
                ...
                if self.first_detection_time is None:
                    ...
                    self.get_logger().info(
                        f"First box detected! Grace period started ({self.grace_period}s)."
                    )

                # 기능이 켜져 있고, 유예 시간(3초)이 지났다면 등록 거부
                if self.lock_new_boxes and (
                    time.time() - self.first_detection_time > self.grace_period
                ):
                    box_to_id[i] = None
                else:
        ...
        for i, det_box in enumerate(current_frame_boxes):
            ...
            if not is_clipped:
                ...
                if np.sum(valid_mask) > 100:  # Need enough points for RANSAC
                    ...
                    u, v = np.meshgrid(
                        np.arange(safe_x1, safe_x2), np.arange(safe_y1, safe_y2)
                    )
                    ...
                    cam_pos = np.array(
                        [
                            t_base_cam.transform.translation.x,
            ...
            # --- 2. Continuous Reconstruction (Clipped, use Ratio) ---
                ...
                patch_depth = cv_depth[
                    max(0, vc_y - 5) : min(self.img_height, vc_y + 5),

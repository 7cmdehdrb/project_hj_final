...
class QuestTeleopNode(Node):
    ...
    def pose_cb(self, msg):
        ...
        R_robot_init = R_robot[:3, :3]

        # ======================
        # Position (EE frame 기준 적용)
        # ======================
        scaling = 1.0 # 스케일링을 다시 1.0(1:1 매핑)으로 복구하거나 원하시는 값으로 하세요
        delta_ctrl = (pos - self.start_ctrl_pos) * scaling
        
        # 컨트롤러의 이동(delta_ctrl)을 EE frame에서의 이동으로 해석하여 Base frame 벡터로 변환
        delta_base = R_robot_init @ delta_ctrl
        ...
        # ======================
        target_pose = [
            float(self.start_robot_pose[0] + delta_base[0]),
            float(self.start_robot_pose[1] + delta_base[1]),
            float(self.start_robot_pose[2] + delta_base[2]),
            float(aa[0]),
            float(aa[1]),
            float(aa[2])
        ]
        ...
        try:
            ...
            if q_target:
                ...
                # q[3] 좌표계상 +pi 또는 -pi (180도)에 가까워지면 툴 플랜지와 하박이 Clamping 됩니다.
                q3_mod = abs(q_target[3]) % (2 * 3.14159)
                if abs(q3_mod - 3.14159) < 0.8: # 역방향으로 180도 중심 +-45도 범위 이내 진입 제어
                ...
                if q_diff > 0.5: # 한 프레임만에 조인트가 0.5 라디안(약 28도) 이상 튀는 비정상 해 기각
